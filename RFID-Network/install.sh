...
# Change the password
echo -e "${COL_LB} It is recommended that you change the password"
...
# Update the system
echo -e  "${COL_LB} Updating the Raspberry Pi's software"
sudo apt-get -y update && sudo apt-get -y upgrade
echo ""

# Configure /boot/config.txt
echo -e "${COL_LB} Editing /bost/config.txt (raspi-config)"
...
echo "    - -> Pin 9 (Ground)"
echo -e "${COL_LB} Editing RTC files"
...
# Setup Node.js
echo -e "${COL_LB} Installing node.js"
...
echo -e "${COL_LB} Granting priveleges to items in server folder"
sudo chmod 777 server/outputCSV
chmod +x server.sh
cd server

echo -e "${COL_LB} Creating node app"
#printf 'rfidnetwork\n1.0.0\nrfidnetwork\napp.js\n \n \n \n \n \nyes\n' | npm init
npm init -y
npm install express mysql socket.io winston

echo -e "${COL_LB} Installing nRF24 libraries"
cd RFIDNetworkPi
...
echo -e "${COL_LB} Installing RF24 python wrapper"
cd RFIDNetworkPi/rf24libs/RF24/pyRF24/
sudo python setup.py build
sudo python setup.py install

echo -e "${COL_LB} Installing RF24Network python wrapper"
cd RFIDNetworkPi/rf24libs/RF24/pyRF24/pyRF24Network
sudo python setup.py build
sudo python setup.py install

echo -e "${COL_LB} Installing RF24Mesh python wrapper"
cd ..
cd RFIDNetworkPi/rf24libs/RF24/pyRF24/pyRF24Mesh
sudo python setup.py build
sudo python setup.py install

echo -e "${COL_LB} Creating rfidnetwork service."
...
# Setup MySQL
echo -e "${COL_LB} Installing MySQL(MariaDB)"
...
echo -e "${COL_LB} Installing dnsmasq and hostapd"
sudo apt-get -y install dnsmasq hostapd
sudo systemctl stop dnsmasq
sudo systemctl stop hostapd

echo -e "${COL_LB} Configuring /etc/dhcpcd.conf"
...
echo -e "${COL_LB} Configuring /etc/dnsmasq.conf"
sudo touch /etc/dnsmasq.conf
sudo mv /etc/dnsmasq.conf /etc/dnsmasq.conf.orig 
sudo cp /home/pi/RFID-Network/server/ap/dnsmasq.conf /etc/dnsmasq.conf

echo -e "${COL_LB} Configuring /etc/hostapd/hostapd.conf"
sudo cp /home/pi/RFID-Network/server/ap/hostapd.conf /etc/hostapd/hostapd.conf

echo -e "${COL_LB} Configuring /etc/default/hostapd"
sudo sed -i 's/#DAEMON_CONF=""/DAEMON_CONF="\/etc\/hostapd\/hostapd.conf"/g' /etc/default/hostapd

echo -e "${COL_LB} Configuring network tables"
